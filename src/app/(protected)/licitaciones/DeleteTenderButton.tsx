...
import { deleteTender } from "./actions";
...
export default function DeleteTenderButton({ tenderId, title }: Props) {
  ...
  function onDelete() {
    ...
    startTransition(async () => {
      try {
        await deleteTender(formData);
        setOpen(false);
        router.refresh();
      } catch (err) {
        console.error(err);
        setError("No se pudo eliminar la licitación");
      }
    });
  }
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-700">
            ¿Seguro que deseas eliminar la licitación{" "}
