...
export default function EditInsurerButton({ insurer }: Props) {
  ...
  async function onSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      try {
        await updateInsurer(formData);
        setOpen(false);
        router.refresh();
      } catch (err) {
        console.error(err);
        setError("No se pudo actualizar la aseguradora");
      }
    });
  }
        ...
        <form action={onSubmit} className="space-y-4">
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700">
            <input
              type="checkbox"
              name="isActive"
              defaultChecked={insurer.isActive}
              disabled={pending}
            />
            Aseguradora activa
          </label>
