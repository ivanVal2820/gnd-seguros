...
import { deleteInsurer } from "./actions";
...
export default function DeleteInsurerButton({ id, name }: Props) {
  ...
  function onDelete() {
    ...
    startTransition(async () => {
      try {
        await deleteInsurer(formData);
        setOpen(false);
        router.refresh();
      } catch (err) {
        console.error(err);
        setError("No se pudo eliminar la aseguradora");
      }
    });
  }
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-700">
            ¿Seguro que deseas eliminar la aseguradora{" "}
